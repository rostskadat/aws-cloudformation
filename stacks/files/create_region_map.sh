...
#
echo "  RegionMap: "
aws --region eu-west-1 ec2 describe-regions --query 'Regions[*].{Name:RegionName}' --output text|sort -u| while read region; do
    latest_ami_name=$(aws --region $region ec2 describe-images --owners amazon --filters "Name=name,Values=amzn-ami-hvm-$(date +%Y)*-gp2" "Name=architecture,Values=x86_64" "Name=root-device-type,Values=ebs" --query 'Images[*].Name'|grep amzn|sort -r|head -1|tr -d '"'|xargs|sed -e 's/,//g')
    echo "    $region: "
